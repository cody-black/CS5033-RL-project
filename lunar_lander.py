...
class LunarLander(gym.Env, EzPickle):
    ...
    zone_move = False
    limited_fuel = False
    fuel = 30 # Initial amount of fuel that the lander has.
    rand_diff = 0
    ...
    def step(self, action):
        ...
        shaping = \
            ...
            - 100*abs(state[4]) - 30*abs(state[5]) + 10*state[6] + 10*state[7]  # And ten points for legs contact, the idea is if you
...
class LunarLanderMoreRandStart(LunarLander):
    rand_diff = 500 # Amount added to the maximum random starting velocity
